...
function initPWAInstall() {
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // すでに拒否されていなければ表示
    if (!pwaInstallDismissed && !localStorage.getItem('pwaInstallDismissed')) {
      setTimeout(() => {
        showPWAInstallModal();
      }, 3000); // 3秒後に表示
    }
  ...
  if (pwaInstallClose) {
    pwaInstallClose.addEventListener('click', hidePWAInstallModal);
  }
...
function showPWAInstallModal() {
  const modal = document.getElementById('pwa-install-backdrop');
  if (modal) {
